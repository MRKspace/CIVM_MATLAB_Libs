...
headerfilename = filepath('/home/scott/Public/pfiles/20140710/imagingtest/P29696.7');
...
toff_high = 0.95;
ntoff = 20;
toff_vals = linspace(toff_low, toff_high, ntoff);
for toffidx = 1:ntoff
	toff_val = toff_vals(toffidx)
	save('toff_val.mat','toff_val');
	nifty_name = ['toff_' num2str(toff_val) '.nii'];
	if(exist(nifty_name)~=2)
		
		% Read in the file and prepare for generic reconstruction
		[revision, logo] = ge_read_rdb_rev_and_logo(headerfilename);
		[data, traj, weights, header] = GE_Recon_Prep_prewind(headerfilename, ...
			floor(revision), datafilename, toff_val);
		% data = data./weights;
...
toff_val = toff_vals(1);
nifty_name = ['toff_' num2str(toff_val) '.nii'];
nii = load_nii(nifty_name);
