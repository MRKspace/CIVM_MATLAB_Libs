...
function write_agilent_archimedian_spiral_trajectories_angleMethod(filename, nviews, primeplus)
loop_Factor = 1;%nearestPrime(round(nviews/3));
...
x_coords(old_idx) = x_coords(new_idx);
y_coords(old_idx) = y_coords(new_idx);
z_coords(old_idx) = z_coords(new_idx);
r = sqrt(x_coords.^2 + y_coords.^2 + z_coords.^2);

thetas = 180*acos(z_coords./r)/pi;
phis = 180*atan2(y_coords,x_coords)/pi;
...
fprintf(fid,'%s','t11 = ');
for i=1:nviews
    fprintf(fid,'%f ',thetas(i)); %azimuthal (from z)
end
fprintf(fid,'\n');
fprintf(fid,'%s','t12 = ');
for i=1:nviews
    fprintf(fid,'%f ',phis(i)); %radial
end
fprintf(fid,'\n');
fclose(fid);
