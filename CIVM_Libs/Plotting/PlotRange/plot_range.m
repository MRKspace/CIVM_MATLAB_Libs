...
for i=1:num_ranges
	...
	nLines = size(current_range_vals{1},2);
	r1 = current_range_vals{1};
	r2 = current_range_vals{2};
	for j=1:nLines
		current_range_vals = [r1(:,j);
			flipud(r2(:,j))];
		
		% Create patch
		h_range = patch(x_vals_symmetric(:,j),current_range_vals,...
