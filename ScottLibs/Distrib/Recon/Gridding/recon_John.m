...
hdr_offset = 66072;
% hdr_offset = 61464;
recon_data.Npts = 128; %rhnframe_size
...
fseek(fid, hdr_offset, 'bof');

recon_data.Data = fread(fid,inf,precision);
            
% Data is complex (real and imaginery parts alternate)
recon_data.Data = complex(recon_data.Data(1:2:end),recon_data.Data(2:2:end));
length(recon_data.Data)        
rem(length(recon_data.Data),128)
% Separate baselines from raw data
recon_data.Nframes  = length(recon_data.Data(:))/recon_data.Npts;
recon_data.Data = reshape(recon_data.Data,recon_data.Npts,recon_data.Nframes);% Reshape into matrix
...
dcf_type = 1; % 1=Analytical, 2=Hitplane, 3=Voronoi, 4=Itterative, 5=Voronoi+Itterative
