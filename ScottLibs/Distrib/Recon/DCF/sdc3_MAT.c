        ...
        int nrhs, const mxArray *prhs[])
{
    ...
    int nd;
    const int *dims;
    unsigned long *dims_l;
    dataArray_double *coords;
    dataArray_double *pweights;
    int wnd;
    const int *wdims;
    unsigned long *wdims_l;
    dataArray_double *DCF;
    int *odims;
    ...
    /* PARAMS */
    effMtx  = (int) *mxGetPr(prhs[2]);
    numIter = (int) *mxGetPr(prhs[1]);
    ...
    coords = new_dataArray_double(nd,dims_l); /* alloc new dataArray_double */
    ...
    if (prhs[5] != NULL && nrhs == 6)
    {
        ...
        wnd = mxGetNumberOfDimensions(prhs[5]); /* get coordinate dimensions */
        assert( wnd > 0 ); /* check for valid array size */
        
        wdims = mxGetDimensions(prhs[5]);
        wdims_l = (unsigned long *) malloc(sizeof(unsigned long) * wnd);
    ...
    DCF = sdc3grid_main(coords,pweights,numIter,effMtx,osf,verbose);
    
    /* Create an mxArray for the return argument */
    if(verbose) printf("Copying output to mxArray.\n");
    odims = (int*) malloc( sizeof(int)*(nd-1) );
