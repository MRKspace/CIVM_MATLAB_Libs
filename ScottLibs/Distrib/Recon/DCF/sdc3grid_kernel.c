...
dataArray_double *loadKernelTable(unsigned long len)
{
    ...
    out = new_dataArray_double( 1, &len );
                                ...
                                int verbose)                    /* turn verbosity on or off               */
{
    ...
    float norm_rfp;
    double winLen;
    dataArray_double *kernelTable;
    unsigned long *dimensions;
    dataArray_double *grid_tmp;
    double rfp;
    int grid_mat;
    dataArray_double *weights_tmp;
    dataArray_double *out;
            
    /* input params */
    rfp = RADIUS_FOV_PRODUCT; 

    /* get grid matrix sizes */
    grid_mat = (int)(effectiveMatrix * osf);

    /* rfp is defined as 2FOV*kr so osf is relative to 2 */
    norm_rfp = rfp*osf;

    /* scale the coords to fit on the grid including kernel radius */
    winLen = ((float)grid_mat + norm_rfp*2.0)/(float)grid_mat;
    ...
    kernelTable = loadKernelTable((unsigned long)(SDC3_DEFAULT_KERNEL_TABLE_SIZE*osf));
    ...
    /** Allocate temp and output arrays */
        ...
        grid_tmp = new_dataArray_double(3,dimensions);
        free(dimensions);
        ...
        out = new_dataArray_double(coords_in->nd-1,dimensions); /* allocate memory */
        free(dimensions); /* free temp dimensions */

        /* TEMP WEIGHTS ( same number of points as output weights ) */
        weights_tmp = new_dataArray_double(out->nd,out->dimensions);
