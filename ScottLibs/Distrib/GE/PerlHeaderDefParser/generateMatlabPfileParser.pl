...
print WRITEFILE "function $struct_base = $matlab_function(pfile_name,offset, byte_order)\n\n";
print WRITEFILE "if(nargin == 0)\n";
print WRITEFILE "\t[file, path] = uigetfile(\'*.*\', \'Select Pfile\');\n";
print WRITEFILE "\tpfile_name = strcat(path, file);\n";
print WRITEFILE "\toffset = 0; %Assume no offset\n";
print WRITEFILE "\tbyte_order = \'ieee-le\'; %Assume little endian\n";
print WRITEFILE "end\n";
print WRITEFILE "\n% Open the PFile\n";
print WRITEFILE "fid=fopen(pfile_name,\'r\',byte_order);         %Little-Endian format\n";
print WRITEFILE "if (fid == -1)\n";
print WRITEFILE "\terror(sprintf(\'Could not open %s file.\',pfile_name));\n";
print WRITEFILE "end\n\n";
print WRITEFILE "% start at correct offset\n";
				...
				if(defined($pre_comment)){
					...
					if(defined($type_conversions{$c_type})){
						...
						if(defined($matlab_type) & ("char" eq $matlab_type)){
							print WRITEFILE "$struct_base = setfield($struct_base, \'$variable\', char(fread(fid,$number,\'char\',byte_order)));";
						}else{
							print WRITEFILE "$struct_base = setfield($struct_base, \'$variable\', fread(fid,$number,\'$matlab_type\',byte_order));";							
						}
